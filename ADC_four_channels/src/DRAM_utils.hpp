...
#pragma HLS resource variable=cell_ID_to_num_vecs core=RAM_2P_URAM
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            ...
            if (cell_ID_to_num_vecs[cell_ID] % ADC_PE_NUM == 0) {
                compute_iter_per_PE = cell_ID_to_num_vecs[cell_ID] / ADC_PE_NUM;
                last_valid_PE_ID = ADC_PE_NUM - 1;
            }
            else {
                compute_iter_per_PE = cell_ID_to_num_vecs[cell_ID] / ADC_PE_NUM + 1;
                last_valid_PE_ID = cell_ID_to_num_vecs[cell_ID] % ADC_PE_NUM - 1;
            }

            s_scanned_entries_every_cell.write(compute_iter_per_PE);
...
void load_cell_ID(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            s_cell_ID.write(cell_ID_DRAM[query_id * nprobe + nprobe_id]); 
        }
    }
}
