...
# Given a top vector, it can be randomly distributed over all channels
P_in_queue = 1 / N_L1_queue
P_not_in_queue = 1 - 1 / N_L1_queue
...
for k in range(topK):
    ...
    print("k={} vectors are in a single L1 queue, probability={}".format(k, p_distribution[k]))
    p_accumulated_distribution[k] = np.sum(p_distribution)
    print("accumulated probability: ", p_accumulated_distribution[k])
